...
class PDFFindController {
  ...
  _calculateMatch(pageIndex) {
    // _pageContents-->将页面内容（文本项）存储为一个字符串。
    //this._pageContents[pageIndex] --> 提取指定pageIndex的字符串信息
    //_normalize 去掉特殊字符，提取页内容
    let pageContent = this._normalize(this._pageContents[pageIndex]);
    //根据this._state.query查询数据
    ...
    if (!caseSensitive) {//是否区分大小写
      pageContent = pageContent.toLowerCase();
      query = query.toLowerCase();
    }

    if (phraseSearch) {//是否短语搜索
  ...
  _updatePage(index) {
    if (this._selected.pageIdx === index) {
      ...
      /**
       * 如果选择了该页面，则将页面滚动到视图中，然后触发该视图 
       * 渲染页面，添加文本图层。 
       * 构建文本图层后，它将滚动到所选匹配项。
       */
      this._linkService.page = index + 1;
    }
    //触发updatetextlayermatches事件更新textlayer
    // console.debug("_updatePage dispatch-->updatetextlayermatches");
