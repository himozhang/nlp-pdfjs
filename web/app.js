...
import { PDFLoadLabelDataUtils } from './pdf_load_label_data';
...
const DefaultExternalServices = {
  ...
  reportTelemetry(data) {
    console.info("reportTelemetry data:");
    console.info(data);
  },
...
let PDFViewerApplication = {
  ...
  fallback(featureId) {
    console.debug("come in fallback....featureId:"+featureId);
    if (typeof PDFJSDev !== 'undefined' &&
  ...
  load(pdfDocument) {
    ...
    pdfDocument.getMetadata().then(
      ...
      this.loadLabelDataUtils._pdfDocument = pdfDocument;
      let loadLabelDataUtils = this.loadLabelDataUtils;
      
      //发出元数据加载完成事件，所有监听此事件的地方均可做出对应处理
      this.eventBus.dispatch('metaDataLoadFinish', { loadLabelDataUtils});
      console.log("metaDataLoadFinish.............");
  ...
  setInitialView(storedHash, { rotation, sidebarView,
    ...
    console.log("set currentPageNumber page:"+page);
    this.pdfViewer.currentPageNumber = parseInt(page);
...
function webViewerInitialized() {
  ...
  if (typeof PDFJSDev === 'undefined' || PDFJSDev.test('GENERIC')) {
    ...
    let seriesidx = 'seriesidx' in params ? params.seriesidx : null;
    if(seriesidx!==null){
       console.log("app(line:1537) set seriesidx :"+seriesidx);
       document.getElementById("seriesIdx").value=seriesidx;
    }
...
function webViewerTextLayerRendered(evt) {
  if (typeof PDFJSDev !== 'undefined' &&
    ...
    console.debug("webViewerTextLayerRendered call fallback....featureId is null");
