...
class TextLayerBuilder {
  ...
  render(timeout = 0) {
    ...
    this.textLayerRenderTask.promise.then(() => {
      ...
      this.updateMatches();
      this.eventBus.dispatch('updateMatchesFinish', {
        source: this
      });
    }, function (reason) {
  ...
  renderMatches(matches) {
    ...
    // console.debug("renderMatches  ---->matches");
    // console.debug(matches);
    // console.debug("renderMatches  ---->this.textDivs");
    // console.debug(this.textDivs);
  ...
  _bindEvents() {
    ...
    _boundEvents.updateMatchesFinish = (evt) => {

      let textLayerBuilder = evt.source;
      let pageIndex = textLayerBuilder.pageNumber;
      let loadLabelDataUtils = this.loadLabelDataUtils;
      if(loadLabelDataUtils){
        console.info("text_layer_builder.updateMatchesFinish 调用loadLabelDataUtils.matchQuery 进行标注,当前pageIndex为--->"+pageIndex);
        
        let textDivs =textLayerBuilder.textDivs;
        let textContentItemsStr = textLayerBuilder.textContentItemsStr;
        
        // loadLabelDataUtils._globalConverMatches(pageIndex,textDivs,textContentItemsStr);
        loadLabelDataUtils.matchQuery(pageIndex,textDivs,textContentItemsStr);
      }else{
        //console.error("loadLabelDataUtils对象未实例化，text_layer_builder.updateMatchesFinish调用loadLabelDataUtils.matchQuery标注失败！！当前pageIndex为--->"+pageIndex);
      }
      /**
       * 
        pageNumber: this.pageNumber,
       ...
       */
      // const loadLabelDataUtils = new PDFLoadLabelDataUtils({
      //   textLayerBuilder,
      //   eventBus,
      // });
    };
    ...
    //PDF元数据加载完成后触发
    _boundEvents.metaDataLoadFinish = (params) => {
      if(!this.loadLabelDataUtils){
        let loadLabelDataUtils = params.loadLabelDataUtils;
        this.loadLabelDataUtils = loadLabelDataUtils;
        loadLabelDataUtils.initPDFLoadLabelDataUtils();
        loadLabelDataUtils._extractText();
      }
    };

    //每一页渲染完成后，均触发此事件,页面渲染完成后，不在触发
    _boundEvents.textLayerRendered = (params) => {
        
    };

    eventBus.on('pagecancelled', _boundEvents.pageCancelled);
    eventBus.on('updatetextlayermatches', _boundEvents.updateTextLayerMatches);
    eventBus.on('metaDataLoadFinish', _boundEvents.metaDataLoadFinish);
    //每一页完成update match之后调用此方法,页面渲染完成后，不在触发
    eventBus.on('updateMatchesFinish', _boundEvents.updateMatchesFinish);
    //每一页渲染完成后，均触发此事件
    eventBus.on('textlayerrendered',_boundEvents.textLayerRendered);
  }
